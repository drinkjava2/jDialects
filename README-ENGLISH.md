...
Download and put file "jdialects-1.0.5.jar" in class folder, or add below in pom.xml: 
```
    <dependency>  
...
*)pojoField() method: mark a column be mapped to a Java POJO's field, this is designed for ORM tool, why not name it "entityField()"? because entityField is too common to be noticed. 
