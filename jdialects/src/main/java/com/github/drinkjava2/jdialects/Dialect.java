...
/**
 * jDialects is a small project which extract all Hibernate's dialects into a
 * separated small jar, usually jDialects is used for build pagination SQL and
 * DDL SQL for mutiple databases, currently jDialects support 75 kinds database
 * dialects include SQLLite and Access.<br/>
 * 
 * jDialects is built for jSqlBox project, but it can be used for any project
 * which use native SQL and need dialects features like pagination, if you are
 * using JDBC/JDBCTemplate/DbUtils..., you may need this tool.
 * 
 * jDialects is developed on Java8 but build on JDK1.6 to support more older
 * Java versions.
 * 
 ...
 * you want compare database's differences in detail.
 * 
 * <pre>
 * Use of jDialect: 
 * Download and put this "jdialects-1.0.0.jar" jar in project lib folder or add below lines in pom.xml:
 *   <dependency>
...
public enum Dialect {
	...
	private String putParamters(Type type, String value, int... lengths) {
		...
		int i = 0;
		if (value.contains("$l"))
			value = StrUtils.replace(value, "$l", String.valueOf(lengths[i++]));
		if (value.contains("$p"))
			value = StrUtils.replace(value, "$p", String.valueOf(lengths[i++]));
		if (value.contains("$s"))
			value = StrUtils.replace(value, "$s", String.valueOf(lengths[i++]));
		return value;
	}
