...
import com.github.drinkjava2.alldialects.StringUtility;
	...
	public void exportDialectTypeNames() {
		...
		for (Class<? extends Dialect> class1 : dialects) {
			...
			String insertSQL = "insert into tb_typeNames ("//
					...
					+ "t_DATALINK," + empty(getTypeNameDefString(t, (Types.DATALINK)))//
					...
					+ "t_REF," + empty(getTypeNameDefString(t, (Types.REF)))//
		...
		for (Map<String, Object> map : lst) {
			sb.append("case \"").append(map.get("dialect")).append("\": {\r\n");

			for (Entry<String, Object> entry : map.entrySet()) {
				String key = entry.getKey();
				key = StringUtility.replace(key, "T_", "$");
				key = StringUtility.replace(key, "t_", "$");
				String value = "" + entry.getValue();
				if (!"N/A".equals(value) && !"LINE".equals(key) && !"line".equals(key) && !"DIALECT".equals(key)
						&& !"dialect".equals(key)) {
					sb.append("typeMappings.put(\"" + key + "\", \"" + value + "\");\r\n");
				}
			}
