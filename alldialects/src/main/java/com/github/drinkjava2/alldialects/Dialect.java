...
public enum Dialect {
	...
	private void initializePaginSqlTemplate() {// NOSONAR
		...
		case "MySQLMyISAMDialect":
			paginSQLTemplate = "select $BODY limit $0BASE_OFFSET, $PAGESIZE";
			break;
		case "H2Dialect":
		...
		case "SQLiteDialect":
			paginSQLTemplate = "select $BODY limit $PAGESIZE offset $0BASE_OFFSET";
			break;
		case "Ingres10Dialect":
		case "Ingres9Dialect":
			paginSQLTemplate = "select $BODY offset $0BASE_OFFSET fetch first $PAGESIZE rows only";
			break;
		case "DerbyDialect":
		...
		case "SQLServer2012Dialect":
			paginSQLTemplate = "select $BODY offset $0BASE_OFFSET rows fetch next $PAGESIZE rows only";
			break;
		case "InterbaseDialect":
			paginSQLTemplate = "select $BODY rows $1BASE_ROW_START to $1BASE_ROW_END";
			break;
		case "DB2400Dialect":
		case "DB2Dialect":
			paginSQLTemplate = "select * from ( select inner2_.*, rownumber() over(order by order of inner2_) as rownumber_ from ( select $BODY fetch first $1BASE_ROW_END rows only ) as inner2_ ) as inner1_ where rownumber_ > $0BASE_OFFSET order by rownumber_";
			break;
		case "Oracle8iDialect":
		case "OracleDialect":
			paginSQLTemplate = "select * from ( select row_.*, rownum rownum_ from ( select $BODY ) row_ ) where rownum_ <= $1BASE_ROW_END and rownum_ > $0BASE_OFFSET";
			break;
		case "DataDirectOracle9Dialect":
		case "Oracle10gDialect":
		case "Oracle9Dialect":
		case "Oracle9iDialect":
			paginSQLTemplate = "select * from ( select row_.*, rownum rownum_ from ( select $BODY ) row_ where rownum <= $1BASE_ROW_END) where rownum_ > $0BASE_OFFSET";
			break;
		...
		case "SQLServer2008Dialect":
			paginSQLTemplate = "WITH query AS (SELECT TMP_.*, ROW_NUMBER() OVER (ORDER BY CURRENT_TIMESTAMP) as ROW_NUM_ FROM ( select ($DISTINCT) TOP($PAGESIZE) $BODY ) TMP_ ) SELECT $FIELDS_OR_ALIAS FROM query WHERE ROW_NUM_ >= $0BASE_OFFSET AND ROW_NUM_ < 1BASE_ROW_END";
			break;
		default:
			paginSQLTemplate = NOT_SUPPORT;
		}
