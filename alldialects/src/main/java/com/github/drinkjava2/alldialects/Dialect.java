 ...
 * source code lines are generated by source code generator
 * 
 * @author Yong Zhu
 * @version 1.0.0
 * @since 1.0.0
...
public enum Dialect {
	...
	private void initializePaginSqlTemplate() {// NOSONAR
		...
		case "DB2400Dialect":
		case "DB2Dialect":
			paginSQLTemplate = "select * from ( select inner2_.*, rownumber() over(order by order of inner2_) as rownumber_ from ( select $BODY fetch first $row_end_base0 rows only ) as inner2_ ) as inner1_ where rownumber_ > $skip_rows order by rownumber_";
			break;
		case "Oracle8iDialect":
		case "OracleDialect":
			paginSQLTemplate = "select * from ( select row_.*, rownum rownum_ from ( select $BODY ) row_ ) where rownum_ <= $ROW_END_BASE0 and rownum_ > $SKIP_ROWS";
			break;
		case "DataDirectOracle9Dialect":
		case "Oracle10gDialect":
		case "Oracle9Dialect":
		case "Oracle9iDialect":
			paginSQLTemplate = "select * from ( select row_.*, rownum rownum_ from ( select $BODY ) row_ where rownum <= $ROW_END_BASE0) where rownum_ > $SKIP_ROWS";
			break;
		...
		case "SQLServer2008Dialect":
			paginSQLTemplate = "WITH query AS (SELECT TMP_.*, ROW_NUMBER() OVER (ORDER BY CURRENT_TIMESTAMP) as ROW_NUM_ FROM ( select ($DISTINCT) TOP($ROW_END_BASE0) $BODY ) TMP_ ) SELECT $FIELDS_OR_ALIAS FROM query WHERE ROW_NUM_ >= $ROW_BEGIN_BASE1 AND ROW_NUM_ < $ROW_END_BASE1";
			break;
		default:
			paginSQLTemplate = NOT_SUPPORT;
		}
