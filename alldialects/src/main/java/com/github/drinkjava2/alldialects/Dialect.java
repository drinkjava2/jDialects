...
/**
 * AllDialects is a small Java tool collect all database's dialects, usually
 * used for build pagination SQL. <br/>
 * 
 * AllDialects is built for jSqlBox project, but it can be used any where which
 * need support all dialects in SQL
 * 
 * Note: Do not modify source code manually, most of this file is generated by
...
public enum Dialect {
	...
	private void initializePaginSqlTemplate() {// NOSONAR
		...
		case "MySQLMyISAMDialect":
			paginSQLTemplate = "$SQL limit $0BASE_OFFSET, $PAGESIZE";
			break;
		case "H2Dialect":
		...
		case "SQLiteDialect":
			paginSQLTemplate = "$SQL limit $PAGESIZE offset $0BASE_OFFSET";
			break;
		case "Ingres10Dialect":
		case "Ingres9Dialect":
			paginSQLTemplate = "$SQL offset $0BASE_OFFSET fetch first $PAGESIZE rows only";
			break;
		case "DerbyDialect":
		...
		case "SQLServer2012Dialect":
			paginSQLTemplate = "$SQL offset $0BASE_OFFSET rows fetch next $PAGESIZE rows only";
			break;
		case "InterbaseDialect":
			paginSQLTemplate = "$SQL rows $1BASE_ROW_START to $1BASE_ROW_END";
			break;
		case "Cache71Dialect":
		case "DB2390Dialect":
		...
		case "DB2400Dialect":
		case "DB2Dialect":
			paginSQLTemplate = "select * from ( select inner2_.*, rownumber() over(order by order of inner2_) as rownumber_ from ( $SQL fetch first $1BASE_ROW_END rows only ) as inner2_ ) as inner1_ where rownumber_ > $0BASE_OFFSET order by rownumber_";
			break;
		case "Oracle8iDialect":
		case "OracleDialect":
			paginSQLTemplate = "select * from ( select row_.*, rownum rownum_ from ( $SQL ) row_ ) where rownum_ <= $1BASE_ROW_END and rownum_ > $0BASE_OFFSET";
			break;
		case "DataDirectOracle9Dialect":
		case "Oracle10gDialect":
		case "Oracle9Dialect":
		case "Oracle9iDialect":
			paginSQLTemplate = "select * from ( select row_.*, rownum rownum_ from ( $SQL ) row_ where rownum <= $1BASE_ROW_END) where rownum_ > $0BASE_OFFSET";
			break;
		case "SQLServer2005Dialect":
		case "SQLServer2008Dialect":
			paginSQLTemplate = "SELECT * FROM (SELECT ROW_NUMBER() OVER($ORDER_BY_ONLY) AS ROW__NM, $NO_ORDER_BODY) TMP_TB WHERE ROW__NM BETWEEN $1BASE_ROW_START AND $1BASE_ROW_END";
			break;
		case "FirebirdDialect":
			paginSQLTemplate = "select first $PAGESIZE skip $0BASE_OFFSET $BODY";
			break;
		case "HSQLDialect":
			paginSQLTemplate = "select limit $0BASE_OFFSET $PAGESIZE $BODY";
			break;
		case "Informix10Dialect":
			paginSQLTemplate = "select SKIP $0BASE_OFFSET FIRST $PAGESIZE $BODY";
			break;
		default:
			paginSQLTemplate = NOT_SUPPORT;
		}
	...
	private void initializePaginFirstOnlySqlTemplate() {// NOSONAR
		...
		case "Ingres9Dialect":
		case "Oracle12cDialect":
			paginFirstOnlyTemplate = "$SQL fetch first $PAGESIZE rows only";
			break;
		case "RDMSOS2200Dialect":
			paginFirstOnlyTemplate = "$SQL fetch first $PAGESIZE rows only ";
			break;
		...
		case "PostgreSQLDialect":
		case "SQLiteDialect":
			paginFirstOnlyTemplate = "$SQL limit $PAGESIZE";
			break;
		case "SQLServer2012Dialect":
			paginFirstOnlyTemplate = "$SQL offset 0 rows fetch next $PAGESIZE rows only";
			break;
		case "InterbaseDialect":
			paginFirstOnlyTemplate = "$SQL rows $PAGESIZE";
			break;
		case "FrontBaseDialect":
		case "JDataStoreDialect":
		case "MckoiDialect":
		...
		case "Oracle9iDialect":
		case "OracleDialect":
			paginFirstOnlyTemplate = "select * from ( $SQL ) where rownum <= $PAGESIZE";
			break;
		case "FirebirdDialect":
	 ...
	 * A special note: for sql2005 and sql2008, "order by" should write alias
	 * name if exist like below:<br/>
