...
public enum Dialect {
	...
	private void initializePaginSqlTemplate() {// NOSONAR
		...
		case "MySQLMyISAMDialect":
			paginSQLTemplate = "select $BODY limit $OFFSET_BASE0, $PAGESIZE";
			break;
		case "H2Dialect":
		...
		case "SQLiteDialect":
			paginSQLTemplate = "select $BODY limit $PAGESIZE offset $OFFSET_BASE0";
			break;
		case "Ingres10Dialect":
		case "Ingres9Dialect":
			paginSQLTemplate = "select $BODY offset $OFFSET_BASE0 fetch first $PAGESIZE rows only";
			break;
		case "DerbyDialect":
		...
		case "SQLServer2012Dialect":
			paginSQLTemplate = "select $BODY offset $OFFSET_BASE0 rows fetch next $PAGESIZE rows only";
			break;
		case "InterbaseDialect":
			paginSQLTemplate = "select $BODY rows $OFFSET_BASE0 to $PAGESIZE";
			break;
		case "DB2400Dialect":
		case "DB2Dialect":
			paginSQLTemplate = "select * from ( select inner2_.*, rownumber() over(order by order of inner2_) as rownumber_ from ( select $BODY fetch first $ROW_END_BASE0 rows only ) as inner2_ ) as inner1_ where rownumber_ > $OFFSET_BASE0 order by rownumber_";
			break;
		case "Oracle8iDialect":
		case "OracleDialect":
			paginSQLTemplate = "select * from ( select row_.*, rownum rownum_ from ( select $BODY ) row_ ) where rownum_ <= $ROW_END_BASE0 and rownum_ > $OFFSET_BASE0";
			break;
		case "DataDirectOracle9Dialect":
		case "Oracle10gDialect":
		case "Oracle9Dialect":
		case "Oracle9iDialect":
			paginSQLTemplate = "select * from ( select row_.*, rownum rownum_ from ( select $BODY ) row_ where rownum <= $ROW_END_BASE0) where rownum_ > $OFFSET_BASE0";
			break;
		case "FirebirdDialect":
			paginSQLTemplate = "select first $PAGESIZE skip $OFFSET_BASE0 $BODY";
			break;
		case "HSQLDialect":
			paginSQLTemplate = "select limit $OFFSET_BASE0 $PAGESIZE $BODY";
			break;
		case "Informix10Dialect":
			paginSQLTemplate = "select SKIP $OFFSET_BASE0 first $PAGESIZE $BODY";
			break;
		case "SQLServer2005Dialect":
		case "SQLServer2008Dialect":
			paginSQLTemplate = "WITH query AS (SELECT TMP_.*, ROW_NUMBER() OVER (ORDER BY CURRENT_TIMESTAMP) as ROW_NUM_ FROM ( select ($DISTINCT) TOP($ROW_END_BASE0) $BODY ) TMP_ ) SELECT $FIELDS_OR_ALIAS FROM query WHERE ROW_NUM_ >= $OFFSET_BASE1 AND ROW_NUM_ < $ROW_END_BASE1";
			break;
		default:
			paginSQLTemplate = NOT_SUPPORT;
		}
	...
	private void initializePaginFirstOnlySqlTemplate() {// NOSONAR
		...
		case "Ingres9Dialect":
		case "Oracle12cDialect":
			paginFirstOnlyTemplate = "select $BODY fetch first $ROW_END_BASE0 rows only";
			break;
		case "RDMSOS2200Dialect":
			paginFirstOnlyTemplate = "select $BODY fetch first $ROW_END_BASE0 rows only ";
			break;
		...
		case "PostgreSQLDialect":
		case "SQLiteDialect":
			paginFirstOnlyTemplate = "select $BODY limit $ROW_END_BASE0";
			break;
		case "SQLServer2012Dialect":
			paginFirstOnlyTemplate = "select $BODY offset 0 rows fetch next $ROW_END_BASE0 rows only";
			break;
		case "InterbaseDialect":
			paginFirstOnlyTemplate = "select $BODY rows $ROW_END_BASE0";
			break;
		case "Cache71Dialect":
		case "SQLServerDialect":
			paginFirstOnlyTemplate = "select ($DISTINCT) top $ROW_END_BASE0 $BODY";
			break;
		case "SQLServer2005Dialect":
		case "SQLServer2008Dialect":
			paginFirstOnlyTemplate = "select ($DISTINCT) TOP($ROW_END_BASE0) $BODY";
		...
		case "Oracle9iDialect":
		case "OracleDialect":
			paginFirstOnlyTemplate = "select * from ( select $BODY ) where rownum <= $ROW_END_BASE0";
			break;
		case "FirebirdDialect":
		case "Informix10Dialect":
		case "InformixDialect":
		case "IngresDialect":
		case "TimesTenDialect":
			paginFirstOnlyTemplate = "select first $ROW_END_BASE0 $BODY";
			break;
		case "HSQLDialect":
			paginFirstOnlyTemplate = "select top $ROW_END_BASE0 $BODY";
			break;
		default:
			paginFirstOnlyTemplate = NOT_SUPPORT;
		}
	...
	/**
	 * This method transfer a universal DDL to database DDL, to compare universal DDL and native DDl, please open this
	 * file "all-dialects-list.csv" in Excel
	 * 
	 * <pre>
	...
	private static String aTopLimitSqlExample(String template) {
		...
		result = StrUtils.replace(result, "$0BASE_OFFSET", "0");
		result = StrUtils.replace(result, "$PAGESIZE", "10");
		result = StrUtils.replace(result, "$1BASE_ROW_END", "10");
		return result;
	}
	...
	public String paginate(int pageNumber, int pageSize, String sql) {// NOSONAR
		...
		String result = StrUtils.replace(paginTemplate, "$0BASE_OFFSET", String.valueOf(offset));
		result = StrUtils.replace(result, "$1BASE_ROW_START", String.valueOf(offsetBase1));
		result = StrUtils.replace(result, "$PAGESIZE", String.valueOf(pageSize));
		result = StrUtils.replace(result, "$1BASE_ROW_END", String.valueOf(maxQty));

		result = StrUtils.replace(result, "$SQL", sql);
		// if have $BODY, replace by real body
		result = StrUtils.replace(result, "$BODY", body);

		// replace $MSORDER_BY to inner temp table's order by fields or alias
		// name
